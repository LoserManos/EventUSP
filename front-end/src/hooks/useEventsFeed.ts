...
import { eventsService, EventFilters } from '../services/eventService';
import { Event } from '../types/event';

export function useEventsFeed(filtrosAtivos: EventFilters, limite: number = 20) {
  ...
  const loadMoreEvents = async (resetPage = false) => {
    ...
    try {
      const response = await eventsService.getEvents(currentPage, limite, filtrosAtivos);
